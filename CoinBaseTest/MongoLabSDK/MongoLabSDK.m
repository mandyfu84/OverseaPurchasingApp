...
#import "JSON.h"
#define APIKEY @"v0KbZnCTNV1mUIziflTd2i932GHY2uAd"
...
-(NSDictionary *) insertCollectionItem:(NSString *) databaseName collectionName:(NSString *) collectionName item:(NSDictionary *) item {
    ...
    NSURLResponse *response = nil;
    NSError *error = nil;
    NSString *urlString = [NSString stringWithFormat:@"https://api.mongolab.com/api/1/databases/%@/collections/%@?apiKey=%@", databaseName, collectionName, APIKEY];
    //NSLog(@"%@ %@ %@",databaseName,collectionName,APIKEY);
    NSURL *url = [[[NSURL alloc] initWithString:urlString] autorelease];
